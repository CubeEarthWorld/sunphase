...
// -------------------------------------------------------
// 2) EnglishTimeParser
//   - Here it's empty, assuming "HH:MM" is delegated to not_language
//   - If you want to handle "4pm" or "17:32" in English here, you can add your own logic
// -------------------------------------------------------
...
class EnglishRefiner implements Refiner {
  ...
  bool _isDateOnly(ParsingResult r) {
    ...
    final dt = r.date;
    // 参考: "today", "tomorrow", "3 days ago" など → 0:00 になりやすい
    return (dt.hour == 0 && dt.minute == 0 && dt.second == 0);
  }

  bool _isTimeOnly(ParsingResult r) {
    // 例: day == reference day ではない可能性もあるが、
    // とりあえず "hour != 0 or minute != 0" などで判断
    final dt = r.date;
    // ここでは day が変わっていても "時刻" とみなす(24h超の補正など)
    // hour, minute が非0 かどうかなどで判定
