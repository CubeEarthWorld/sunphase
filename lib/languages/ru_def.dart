...
class RuDefinitions {
  ...
  static final patterns = [
    ...
    // Calendar week + weekday: воскресенье на следующей неделе
    PatternDef(
      name: 'ru_nextWeekWeekday',
      regex: RegExp(
        r'(?:(?:в\s+)?(понедельник|вторник|среду|четверг|пятницу|субботу|воскресенье)\s+(?:на\s+)?следующей\s+недел[еи]|(?:на\s+)?следующей\s+недел[еи]\s+(?:в\s+)?(понедельник|вторник|среду|четверг|пятницу|субботу|воскресенье))',
        caseSensitive: false,
      ),
      extract: (match, np, ref) {
        ...
        return RawMatch(
          ...
          endIndex: match.end,
          text: match.group(0)!,
          weekday: weekdays[nominativeDay] ?? weekdays[day] ?? 1,
          weekOffset: 1,
          calendarWeek: true,
        );
      },
    ...
    PatternDef(
      name: 'ru_dayOffset',
      regex: RegExp(
        r'через\s+(\d+)\s+дня?|дн[ея]?\s+назад\s+(\d+)|(\d+)\s+дн[ея]?\s+назад',
      ),
      extract: (match, np, ref) {
        int? days;
        bool isAhead = true;
    ...
    PatternDef(
      name: 'ru_weekOffset',
      regex: RegExp(
        r'через\s+(\d+)\s+недел[ьи]|недел[иьюю]\s+назад\s+(\d+)|(\d+)\s+недел[иьюю]\s+назад',
      ),
      extract: (match, np, ref) {
        int? weeks;
        bool isAhead = true;
        ...
        return RawMatch(
          ...
          endIndex: match.end,
          text: match.group(0)!,
          dayOffset: isAhead ? ((weeks) * 7) : -((weeks) * 7),
        );
      },
    ),
    ...
    PatternDef(
      ...
      regex: RegExp(
        r'(?:на\s+)?(следующей|прошлой|этой)\s+недел[еи]',
        caseSensitive: false,
      ),
      extract: (match, np, ref) {
