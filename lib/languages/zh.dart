...
class ChineseDateParser implements Parser {
  ...
  List<ParsingResult> parse(String text, DateTime referenceDate) {
    ...
    // ------------------------------
    // 数字 + 天/周/个月/年 (+ 前|后) 例如: "3天后", "2周前", "5个月后", "2年后"
    // 其中也支持部分汉数字 (一二三四五六七八九十)
    // ------------------------------
    ...
    // ------------------------------
    // 标准的日期字符串 (如 ISO8601 等) —— 当整个文本为此格式时解析
    // ------------------------------
    try {
    ...
    // ------------------------------
    // 追加: 单独的 "◯日" 或 "◯号" —— 当不含有 "月" 时，视为最近的某月的该日
    // ------------------------------
    // 注意：因中文中没有空格分隔，故不使用 \b，而是利用否定先行 (?<!月)
    final RegExp singleDayPattern = RegExp(r'(?<!月)(\d{1,2})(日|号)');
