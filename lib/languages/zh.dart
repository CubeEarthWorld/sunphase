...
class ZhRelativeParser extends BaseParser {
  ...
  List<ParsingResult> parse(String text, ParsingContext context) {
    ...
    if (mCombined != null) {
      ...
      int minute = mCombined.group(4) != null ? int.parse(mCombined.group(4)!) : 0;
      ...
      if (text.contains("明天")) {
        ...
        results.add(ParsingResult(index: text.indexOf("明天"), text: "明天", date: tomorrow));
      }
      if (text.contains("后天")) {
        DateTime dayAfterTomorrow =
        DateTime(ref.year, ref.month, ref.day).add(Duration(days: 2));
        results.add(ParsingResult(index: text.indexOf("后天"), text: "后天", date: dayAfterTomorrow));
      }
      if (text.contains("昨天")) {
        DateTime yesterday =
        DateTime(ref.year, ref.month, ref.day).subtract(Duration(days: 1));
        results.add(ParsingResult(index: text.indexOf("昨天"), text: "昨天", date: yesterday));
      }
    }
    ...
    // 处理“下周”＋星期（例如 “下周四”）
    RegExp regWeek = RegExp(r'下周(?:[星期周])([一二三四五六日])');
...
/// 中文绝对表达式解析器
/// （例如：“4月26日4时8分”，“2028年5月1日”，“今年12月31日”，“三月四号”等）
/// 同时支持 "明年1月1日" 以及 "下个月15号"、"上个月20号"
class ZhAbsoluteParser extends BaseParser {
  @override
  List<ParsingResult> parse(String text, ParsingContext context) {
    List<ParsingResult> results = [];
    // 支持带年份前缀的形式，例如 "明年1月1日"
    RegExp regExp = RegExp(r'(?:(明年|去年|今年))?(\d{1,2})月(\d{1,2})日(?:\s*(\d{1,2})时(?:\s*(\d{1,2})分)?)?');
    ...
    // 漢字形式：例如 "三月四号"
    RegExp regKanji = RegExp(r'([一二三四五六七八九十]+)月([一二三四五六七八九十]+)[号日]');
    ...
    if(text.contains("下个月")){
      RegExp exp = RegExp(r'下个月(\d{1,2})[号日]');
      RegExpMatch? m = exp.firstMatch(text);
      if(m != null) {
        int day = int.parse(m.group(1)!);
        DateTime nextMonthStart = DateTime(context.referenceDate.year, context.referenceDate.month + 1, 1);
        DateTime candidate = DateTime(nextMonthStart.year, nextMonthStart.month, day);
        results.add(ParsingResult(index: m.start, text: m.group(0)!, date: candidate));
      }
    }
    // 处理 "上个月20号"
    if(text.contains("上个月")){
      RegExp exp = RegExp(r'上个月(\d{1,2})[号日]');
      RegExpMatch? m = exp.firstMatch(text);
      if(m != null) {
        int day = int.parse(m.group(1)!);
        DateTime prevMonth = DateTime(context.referenceDate.year, context.referenceDate.month - 1, 1);
